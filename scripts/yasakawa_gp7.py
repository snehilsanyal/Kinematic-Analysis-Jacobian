...
# -----------------------------
# Define symbolic variables and DH parameters for Yasakawa GP7
# -----------------------------
theta1, theta2, theta3, theta4, theta5, theta6 = sp.symbols('theta1 theta2 theta3 theta4 theta5 theta6', real=True)
theta = [theta1, theta2, theta3, theta4, theta5, theta6]

# Yasakawa GP7 DH parameters: [a, alpha, d, theta]
# (Dimensions converted to meters from mm: 40 mm -> 0.04, 330 mm -> 0.33, etc.)
DH_params = [
    [0.04,      sp.pi/2, 0.33,   theta1],    # Joint 1: S
    [0.445,     0,       0,      theta2],    # Joint 2: L
    [0.04,      sp.pi/2, 0,      theta3],    # Joint 3: U
    [0,        -sp.pi/2, 0.44,   theta4],    # Joint 4: R
    [0,         sp.pi/2, 0,      theta5],    # Joint 5: B
    [0,         0,       0.08,   theta6]     # Joint 6: T
]
...
# -----------------------------
# Sample three joint angles (theta1, theta2, theta3) uniformly over [-pi, pi]
# while holding theta4, theta5, theta6 at 0.
num_samples = 20
...
for t1 in theta_samples:
    for t2 in theta_samples:
        for t3 in theta_samples:
            ...
            # Manipulability index based on the Jacobian
...
fig_workspace.update_layout(
    title='Yaskawa GP7 Workspace (mm)',
    scene=scene_layout
)
fig_workspace.show()
...
fig_singular.update_layout(
    title='Yaskawa GP7 Singular Configurations (mm)',
    scene=scene_layout
)
fig_singular.show()
...
fig_condition.update_layout(
    title='Yaskawa GP7 - Workspace & Condition Index (mm)',
    scene=scene_layout
)
fig_condition.show()
...
fig_manipulability.update_layout(
    title='Yaskawa GP7 - Workspace & Manipulability Index (mm)',
    scene=scene_layout
)
